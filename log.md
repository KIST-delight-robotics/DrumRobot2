...
- 15:53 KST (UTC+9) — simplify pass(wait 제거 + imperative FSM 전환) + decision graph refactor 3단계(cross-turn recovery)
  - 수정 파일: `phil_robot/pipeline/planner.py`, `phil_robot/pipeline/skills.py`, `phil_robot/pipeline/command_validator.py`, `phil_robot/pipeline/motion_resolver.py`, `phil_robot/pipeline/exec_thread.py`, `phil_robot/pipeline/robot_fsm.py`(신규), `phil_robot/pipeline/session.py`, `phil_robot/phil_brain.py`, `phil_robot/eval/brain_probe.py`, `phil_robot/tests/test_planner_benchmark.py`, `phil_robot/TODO.md`, `log.md` / 삭제: `phil_robot/pipeline/robot_graph.py`, `phil_robot/pipeline/state_graph.py`
  - 메모:
    - **Step1 wait 완전 제거**: `wait:<seconds>`는 로봇 명령이 아니라 Python 지연이라 들어냄. planner 예시/allowed_prefixes, motion_resolver의 `wait_then_more` 다단계 파서(`_parse_wait_step_list`/`WAIT_STEP_PATTERN`/`_format_wait_command` + dead 헬퍼들), skills/command_validator(`validate_wait_command`)에서 제거. wait는 이제 validator가 unknown으로 거부. "N도씩 두번"(repeat)은 wait 안 써 생존, "N초 뒤에"만 사라짐. 관련 wait 테스트 4개 삭제. (wait의 정식 자리는 추후 TaskScheduler `after_delay`)
    - **Step2 imperative FSM 전환**: 경량 StateGraph(langgraph shim)는 분기 1~2개에 과해서, Jetson Python 3.8이라 진짜 langgraph 이관도 불가 → `state_graph.py`+`robot_graph.py` 삭제하고 `robot_fsm.py`의 `run_turn`/`build_run_turn`(고정 step 순서 + repair for-loop)으로 교체. `exec_thread`는 wait/cancel/stop_event/cancelled 제거하고 "보내고 on_done()"만 남김(인터럽트 불필요 — wait 없으면 끊을 게 없음). `ingest`→`preprocess`(prefilter만). phil_brain은 `run_turn(user_text)` 호출.
    - **Step3**: ambiguous-followup shortcut 제거(generic planner가 처리), `resolve_clarification_text` 제거(session+brain_probe).
    - **Step4 cross-turn recovery**: `SessionContext`에 `recovery_count`/`pending_intent`/`pending_classifier`(기존 q기반 pending 교체). `run_turn` 진입에서 giveup(`recovery_count>=MAX_RECOVERY=4`면 planner 없이 결정적 리셋) + continuation(pending 활성이면 classify 건너뛰고 원래 도메인/classifier 재사용, pending_intent를 session_summary로 planner에 전달) + 취소어 폐기. validator step에 missing-info 트리거(빈 계획 actionable→repair로 되묻기). update_session이 미해결/해결 판정해 recovery 갱신.
    - 검증: 각 step 컴파일 + 23 단위테스트(wait 4개 제거 후) + stub e2e + 실제 30b. 실제 30b 거부경로 full recovery 확인(`허리 200도`→repair "다른 각도를?"→`30도`→`move:waist,30`). **한계**: bare "허리 돌려"(각도 없음)는 30b가 기본각을 지어내(범위 내라 validator 통과) missing-info가 안 걸림 — planner 순종 문제(blocked-by-lock과 동류), planner 의미해석/resolver 분리에서 구조적 해결 필요.

## 2026-06-10
- 16:54 KST (UTC+9) — repair 도메인 prompt echo 수정 + scenario eval 설계안 기록
  - 수정 파일: `phil_robot/pipeline/planner.py`, `phil_robot/eval/README.md`, `phil_robot/TODO.md`, `log.md`
  - 메모: repair 도메인 prompt의 예시 줄("안전 키 미해제 → 안전 키를 뽑아 달라고 안내")을 30b가 그대로 베껴 speech로 내던(echo) 문제를 고쳤다. 예시 줄을 제거하고 "t는 사용자에게 들려줄 완성된 문장이며 repair_hint 문구를 옮기지 말라"는 zero-shot 지시로 교체. 실제 graph+30b 재구동 확인: 안전키→"안전 키를 해제해야 손을 흔들 수 있어. 안전 키를 확인해 줄 수 있어요?", 연주중→"현재 연주 중이어서 손을 흔들 수 없어요." (echo 사라짐, 자연스러움). happy-path(팔 올려/연주/상태/이름)도 graph 구동으로 repair=0 정상 확인. 또한 graph 구동 multi-turn scenario eval 설계안을 `eval/README.md`와 TODO Next에 기록(구현은 3단계와 함께). 남은 톤 이슈(반말/존대 혼용)는 경미해 보류.

- 16:34 KST (UTC+9) — decision graph refactor 2단계 선회: planner↔validator repair 루프 도입
  - 수정 파일: `phil_robot/pipeline/planner.py`, `phil_robot/pipeline/validator.py`, `phil_robot/pipeline/state_adapter.py`, `phil_robot/pipeline/brain_pipeline.py`, `phil_robot/pipeline/robot_graph.py`, `phil_robot/pipeline/session.py`, `phil_robot/TODO.md`, `log.md`
  - 메모: Jetson smoke 실측에서 "planner self-refuse" 방향의 두 문제(① planner가 좋은 거절 speech를 내면서 명령도 같이 내 validator 안전망이 그 speech를 generic으로 덮는 버그, ② 무거운 관절범위표 prompt가 단순 wave/play를 흔듦)가 드러나, 사용자 원안인 planner↔validator 내부 repair 루프로 선회. (1) planner prompt 슬림화(범위표/self-refuse/q 제거, `JOINT_LIMITS` import 제거) → 흔들림 안정화 확인. (2) `repair` planner 도메인 신설(zero-shot, skills 빈 카테고리): validator 거부 사유를 받아 설명/되묻기 speech만 생성. (3) validator는 speech 클로버 폐기하고 `RepairHint`(failure_code+reason+rejected) 반환; `block_reason_of`(state_adapter 공용)+`_content_failure_code`로 코드 정규화; `build_motion_block_message`/`build_partial_execution_message` 제거. (4) graph에 조건부 엣지 repair 루프(`route_after_validator`, `repair_attempt`/`MAX_REPAIR=2`, repair 도메인 강제) + `fallback` 노드(소진 시 명령 버리고 `FALLBACK_MESSAGE`). q 기반 session pending 비활성화(3단계 pending_intent로 재구성 예정). 검증: stub e2e 3경로 + 실제 graph+30b 구동(안전키/연주중/정상) 통과. 한계: `run_eval`은 brain_probe 단일패스라 repair 루프 미검증 — graph 구동 scenario eval은 후속.

- 16:06 KST (UTC+9) — mic listener의 pre-buffer/침묵 판정 주석 위치 정리 및 최소 클립 이름 변경
  - 수정 파일: `phil_robot/runtime/mic_listener.py`, `log.md`
  - 메모: `MAX_UTTER_FRAMES`를 20초 기준으로 늘리고, `MIN_UTTER_SECONDS`는 값 변경 없이 `MIN_CLIP_SECONDS`로 바꿔 최종 클립 길이 기준임을 드러냈다. 긴 설명 주석은 줄이고 `pre_buffer`, `stream.read`, 볼륨 판정, 프레임 추가, 침묵/최대 길이 종료 지점에 짧은 주석을 붙였다. 검증: `python3 -m py_compile phil_robot/runtime/mic_listener.py`

- 11:54 KST (UTC+9) — decision graph refactor 2단계: validator 안전망化 + planner 단독 메시지 생성 + 관절범위 prompt 배치
  - 수정 파일: `phil_robot/pipeline/planner.py`, `phil_robot/pipeline/validator.py`, `phil_robot/pipeline/state_adapter.py`, `phil_robot/pipeline/failure.py`, `phil_robot/TODO.md`, `log.md`
  - 메모: planner가 막힘/되묻기/범위안내를 직접 생성하도록 전환. `state_adapter._derive_block_reason`로 `block_reason`(safety_key/playing/error/moving/none)을 planner state 요약(동적)에 추가하고, `JOINT_LIMITS`를 command_validator에서 가져와 `JOINT_LIMITS_TEXT`로 `PLANNER_SHARED_RULES`(정적 system prompt prefix)에 넣었다(KV cache 친화). planner prompt에 "각도 없음/범위 초과/block_reason!=none이면 명령 만들지 말고 speech로 되묻기" 규칙 추가, `q` 필드 요구 제거(파서 호환은 유지). validator는 speech 작가 역할을 버리고 안전망化: `build_validated_plan`의 block/partial 문구 캐스케이드를 삭제하고, planner 명령이 전부 거부되는 희귀 케이스만 `SAFETY_NET_FALLBACK`으로 덮는다. `failure.build_motion_block_message` 제거. 검증: stub e2e 3경로(normal/blocked-refuse/outofrange-safetynet) + 27 단위테스트 통과. 실제 LLM의 범위 거절 준수는 ollama 필요라 Jetson eval 대상. 2→3단계 사이 multi-turn stitching 갭은 3단계 pending_intent에서 복구 예정.

- 11:42 KST (UTC+9) — decision graph refactor 1단계: 런타임 파이프라인 노드 분해 + BrainTurnResult eval 이주
  - 수정 파일: `phil_robot/pipeline/brain_pipeline.py`, `phil_robot/pipeline/robot_graph.py`, `phil_robot/pipeline/intent_classifier.py`, `phil_robot/pipeline/session.py`, `phil_robot/phil_brain.py`, `phil_robot/eval/brain_probe.py`(신규), `phil_robot/eval/run_eval.py`, `phil_robot/eval/run_voice_io_benchmark.py`, `phil_robot/eval/planner_json_benchmark.py`, `phil_robot/eval/run_format_compare.py`, `phil_robot/tests/test_planner_benchmark.py`, `phil_robot/eval/README.md`, `phil_robot/TODO.md`, `log.md`
  - 메모: `run_brain_turn`을 step 함수(`build_prefilter_plan`/`classify_step`/`build_direct_answer_plan`/`planner_step`)로 쪼개고, graph를 한 덩어리 `process_node`에서 `ingest→classify→state→direct_answer→planner→validator→execute` 7노드로 분해했다. 노드 사이엔 `PhilState` 딕셔너리 하나만 흐르고 `BrainTurnResult` 진단 래퍼는 런타임에서 제거. classifier 입력에서 robot_state 제거(dead token), state fetch를 classifier 뒤·planner 앞 `state_node`로 이동(cross-turn 복구 대비), 인터럽트는 `ingest_node`로 이동. `phil_brain`은 `app.invoke({"user_text":...})` 후 PhilState의 speech/commands만 읽는 thin 구조로 정리. `run_brain_turn`/`BrainTurnResult`는 삭제가 아니라 `eval/brain_probe.py`로 이주(런타임과 같은 step 함수를 호출하는 eval/benchmark 진단 어댑터). eval/test import re-point 및 `build_classifier_input` 1-arg 정리 완료. 검증: stub LLM/state로 graph e2e 3경로(prefilter/planner/direct_answer) 통과, `tests/test_planner_benchmark.py` 27건 통과. 실제 모델 eval smoke는 ollama 필요라 Jetson 확인 대상. eval 전환 상태는 `eval/README.md` `현재 상태` 섹션에 상세 기록.

- 10:45 KST (UTC+9) — decision graph refactor 확정 설계를 TODO.md Now에 최신화
  - 수정 파일: `phil_robot/TODO.md`, `log.md`
  - 메모: 사용자와의 설계 논의 결과를 반영해 Now 3단계로 재구성했다. 핵심 전제: 턴 내부 planner↔validator 순환 폐기(턴당 planner 1회), 되돌아감은 cross-turn 복구 한 종류로 통일(`recovery_count` 5회 캡, 회차5는 planner 없이 deterministic 리셋), planner가 막힘/되묻기/범위안내 메시지를 직접 생성(관절 범위+막힘 사유를 입력으로), validator는 안전망化(speech override 제거), 관절 범위 셋은 정적이라 `PLANNER_SHARED_RULES` 윗단에 두어 KV prefix cache. pending은 문자열 결합 폐기하고 `pending_intent` context 전달로 전환. 1단계는 `run_brain_turn`/`BrainTurnResult` 분해부터.
